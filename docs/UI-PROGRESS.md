...
| Header creation | The document may only be edited through parts it already has. Creating one means a new `word/headerN.xml`, its content type, its relationship and the `w:headerReference` in the section, and the undo snapshot would have to cover a part that is not media. **Attempted and stopped**, with the obstacle found - see the note below. |
...
### Header creation: attempted, and the obstacle is the snapshot's shape

This was the smallest of the five, so it was tried. The part, the relationship and
the `w:headerReference` are all straightforward - `setReference` on the section
properties already exists, and the layout has laid headers out since Phase 2. What
stopped it is the undo snapshot.

The picture work taught that a part an edit creates must be inside the snapshot, so
the first move was to generalise the media snapshot to every part. That change is
wrong, and the numbering test said so immediately: **parts are restored by three
different paths that do not compose.** Numbering has its own restore, which
recreates `word/numbering.xml` under the snapshot's name; regions have theirs; and
relationships have a third. A generic part restore creates the part first, and then
the specialised restore finds the name taken and allocates `word/numbering1.xml` -
so redo of "apply a list" silently moved the numbering part.

The fix is not to widen the generic path but to give header and footer parts the
restore their siblings already have, which means the region snapshot has to carry
the part and not only its content. That is a contained piece of work, and it is
recorded here rather than guessed at a second time.

Reverted to the media-only scope, and the suite is back to green.
