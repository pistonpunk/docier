...
### Header creation, done - and the obstacle was the model's story set

This one was attempted twice. The first attempt failed on the snapshot; the second
found the real obstacle, which is bigger and which three of the five remaining
items share.

**The model's story set was fixed at load.** `DocumentModel` built its story list
once, in `load()`, and held it `readonly`. So a command could create
`word/header1.xml`, its content type, its relationship and the `w:headerReference`
- and the document would still have no header story, because nothing could add one.
The part would be in the file and invisible to the editor, which is worse than
refusing.

**The fix is two methods.** `adoptStory(kind, partName, element)` appends a story
and rebuilds the id map; `dropStory(partName)` removes one and forgets its subtree.
Nothing else in the model needed to change, because the caches that key off story
identity are all derived per relayout and a *new* story invalidates none of them.

**The snapshot then had to learn to carry a part, and the order matters.**
`RegionSnapshot` captures a region's root; `restoreRegionParts` now removes any
region part that is not in the snapshot - dropping its story first - and recreates
the ones that are missing, adopting the story again. It runs *before*
`restoreRelationships`, because a region story is only reachable through the
relationship that names it, so recreating the part after the relationship would
leave the two out of step for one relayout.

**And `changeRegions` had a hole.** It compared the region stories that existed
before the write, so a write that *added* a region read as no change at all - which
made the command bail out after doing the work. It now also compares the part-name
list before and after.

### Header creation verification, live in the browser

| Check | Measured |
|---|---|
| The Insert tab's Header with no header present | enabled, no reason |
| Clicking it | a header region appears, 0 to 1 |
| Typing | lands in the header, not the body |
| Undo once | the text goes, the header stays |
| Undo twice | the header goes, and the part with it |
| Redo once | the header comes back |
| Redo twice | the text comes back |
| Console and page errors | none |

Two independent entries - one to create, one to type - which is what the two
separate user actions deserve.

### Table of contents, done

A TOC looked like it needed field evaluation, and it does - but the field's result
is not something the *layout* has to compute. It is paragraphs, and the editing
layer can write them.

So the command walks the body for headings, writes a `TOC` field around one
paragraph per heading and nothing else:

    <w:p> begin + instrText " TOC \o "1-3" \h " + separate </w:p>
    <w:p><w:pPr><w:pStyle w:val="TOC1"/></w:pPr> the heading's text </w:p>
    ...
    <w:p> end </w:p>

That is exactly the shape Word writes, which means the document is a real document
with a real TOC in it, and Word's own "update field" would regenerate the entries
rather than choking on a foreign structure. The entries carry Word's `TOC1`,
`TOC2` style ids, so they indent themselves if the document defines those styles
and read as ordinary paragraphs if it does not.

**A heading is one with a heading style or an outline level.** `headingLevelOf`
reads `w:outlineLvl` first - it is the property Word itself consults for a TOC -
and falls back to a style id matching `heading N`, tolerating the `Heading1`,
`heading 1` and `heading_1` spellings. Level 1 is outline level 0, so the two
agree. Tested against both, and against a paragraph that has neither.

The result is deliberately **not** live: it is computed at insert time, and
